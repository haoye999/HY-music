<template>
  <div class="singer-detail" @mousewheel.prevent @touchmove.prevent>
    <music-list
      :bgImg="httpsify(artist.picUrl)"
      :songs="hotSongs"
      :title="artist.name"
    >
      <div class="share">
      ...
      <div class="base-info-container">
        <h2 class="singer-name">
          {{ artist.name
          }}<span v-show="artist.alias && artist.alias.length">
            ({{ artist.alias && artist.alias[0] }})
          </span>
        </h2>
        <button
          class="follow"
          :class="{ followed: artist.followed }"
          @click="_toggleFollow"
        >
          <i
            class="iconfont icon"
            :class="[artist.followed ? 'icon-check-circle' : 'icon-add']"
          ></i
